...
<template>
    ...
    <div class="tabela-de-carros">
        <DataTable v-model:filters="filters" :value=" atualizar" paginator :rows="5">
            <template #header>
                <div>
            ...
            <Column>
                <template #body = "{data}" :v-tooltip="'Almir'" >
                    <Button v-tooltip.left="'Exibir Detalhes'" icon="bi bi-eye" @click=" " text raised  />
                </template>
            </Column>
            ...
            <Column header="Ações"> 
                <template #body="{ data }">
                    <SplitButton :model=" conteudoSplitButton" v-tooltip="'Mais Acões'" size="small" raised text class="split-button" />
                </template>
            </Column>
        </DataTable>
    </div>
